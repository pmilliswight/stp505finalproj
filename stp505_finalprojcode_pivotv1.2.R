...
# returns SxJ matrix
# alpha and beta are clamped to small positive numbers to avoid invalid beta params
...
# if grid approximation fails, we do method of moments estimates for alpha and beta from smoothed proportions ((y+05)/(n+1)) instead
# we just don't want zeroes, forces minimal precision =1 if needed
# generates SxJ theta draws from estimated Beta hyper
# just a pragmatic fallback, it's not great and shouldn't be relied on
state_mom_beta_fallback <- function(state_name, df, S = S_draws) {
  idx_state <- which(df$state == state_name)
  yv <- df$y[idx_state]; nv <- df$n[idx_state]
  J <- length(yv)
  if (J == 0) stop("State not found:", state_name)
  p_obs <- (yv + 0.5) / (nv + 1)          # smoothed empirical proportions
  m <- mean(p_obs); v <- var(p_obs)
  if (is.na(v) || v <= 0) v <- 1e-6
  phi_hat <- pmax((m * (1 - m) / v) - 1, 1) # at least 1
  alpha_hat <- pmax(m * phi_hat, 1e-6)
  beta_hat <- pmax((1 - m) * phi_hat, 1e-6)
  theta_mat <- matrix(NA_real_, nrow = S, ncol = J)
  for (s in seq_len(S)) {
    theta_mat[s, ] <- rbeta(J, shape1 = alpha_hat + yv, shape2 = beta_hat + nv - yv)
  }
  theta_mat
}
...
for (i in seq_along(states)) {
  ...
  theta_mat <- tryCatch(
    refit_state_beta_grid(st, df, S = S_draws, grid_length = grid_length_default,
                          log_ratio_range = c(-4,4), log_sum_range = c(log(0.5), log(200))),
    error = function(e) NULL,
    warning = function(w) NULL
  )
  
  state_results[[st]] <- list(theta_samples = theta_mat)
...
# this computes posterior means and 95% CIs for each county under hierarchical model (theta_draws matrix) and independent (beta(1,1)) model (ind_theta_samples)
# results are numeric vectors aligned to rows of df
# just a saved summary of hierarchical and independent post distributions 
...
# for each post draw we get predicted counts Y_tilde ~ binom(n, theta) for every county (hierarchical and independent)
...
p_waic <- ggplot(waic_df, aes(x = model, y = WAIC)) +
  geom_col() +
  geom_text(aes(label = round(WAIC,1)), vjust = 1.5, color = 'white') +
  labs(title = "WAIC Comparison", y = "WAIC (lower is better)") +
  theme_minimal()
ggsave(paste0(plot_prefix, "waic_comparisonv1.2.pdf"), p_waic, width = 6, height = 4, dpi = dpi)
...
p_hist <- ggplot(df_sel, aes(x = pred)) +
  ...
  labs(title = paste0("Posterior Histogram for ", df$county[sel], " (", df$state[sel], ")"),
       x = "Predicted Vaccination Count", y = "Frequency") +
  theme_minimal()
ggsave(paste0(plot_prefix, "predictive_hist_selectedv1.1.pdf"), p_hist, width = 6, height = 4, dpi = dpi)
...
shrinkage_selected <- snap[order(snap$obs_rate)[1000:1007], ]
...
selected_eff <- bind_rows(shrinkage_selected) %>%
  distinct(FIPS, .keep_all = TRUE) %>%
  slice_head(n = k)
...
p_eff <- ggplot(selected_eff, aes(x = label)) +
  # observed
  geom_point(aes(y = obs_rate), size = 2, color = "black") +
  ...
  geom_point(aes(y = hier_post_mean),
             position = position_nudge(x = 0.2), size = 2, shape = 17,
             color = "steelblue4") +
  
  coord_flip() +
  labs(title = "Model Efficacy Zoom: How Hierarchical Shrinkage Improves Estimates",
       subtitle = paste0("Blue: Hierarchical 95% CI, Gray: Independent 95% CI"),
       x = "", y = "Vaccination proportion") +
  theme_minimal(base_size = 12) +
...
ggsave("shrinkage_model_efficacy_zoomv1.6.pdf", p_eff, width = 10, height = 6, dpi = 300)
print(p_eff)
...
# model efficacy predictive zoom neighborhood
...
selected_neigh_pred <- selected_neigh_pred %>%
  dplyr::mutate(label = paste0(county, " (", state, ")")) %>%
  arrange(n)
...
p_neigh_pred <- ggplot(selected_neigh_pred, aes(x = label)) +
  ...
  coord_flip() +
  labs(
    title = paste0("Predictive Intervals — Selected Counties"),
    subtitle = "Line = 95% predictive interval; cross = predictive mean; dot = observed",
    x = "", y = "Vaccination proportion"
  ) +
  theme_minimal(base_size = 12)

ggsave("predictive_model_efficacy_zoomv1.4.pdf", p_neigh_pred, 
